...
yc_res_map_status_t yc_res_map_parse_object_patch_critter(
    ...
    if (0 == api->fread(&_damage_last_turn, sizeof(uint32_t), 1, file)) { return YC_RES_MAP_STATUS_IO; }
    _damage_last_turn = yc_res_byteorder_uint32(_damage_last_turn);

    uint32_t _combat_state = 0;
    if (0 == api->fread(&_combat_state, sizeof(uint32_t), 1, file)) { return YC_RES_MAP_STATUS_IO; }
    _combat_state = yc_res_byteorder_uint32(_combat_state);

    uint32_t _action_points = 0;
    if (0 == api->fread(&_action_points, sizeof(uint32_t), 1, file)) { return YC_RES_MAP_STATUS_IO; }
    _action_points = yc_res_byteorder_uint32(_action_points);

    uint32_t _damage_flags = 0;
    if (0 == api->fread(&_damage_flags, sizeof(uint32_t), 1, file)) { return YC_RES_MAP_STATUS_IO; }
    ...
    if (0 == api->fread(&_who_hit_me, sizeof(uint32_t), 1, file)) { return YC_RES_MAP_STATUS_IO; }
    ...
    if (0 == api->fread(&hp->value, sizeof(uint32_t), 1, file)) { return YC_RES_MAP_STATUS_IO; }
    ...
    if (0 == api->fread(&pl->value, sizeof(uint32_t), 1, file)) { return YC_RES_MAP_STATUS_IO; }
    ...
    if (0 == api->fread(&rl->value, sizeof(uint32_t), 1, file)) { return YC_RES_MAP_STATUS_IO; }
