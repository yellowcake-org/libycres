...
yc_res_map_status_t yc_res_map_parse_script(
        void *file, const yc_res_io_fs_api_t *io, yc_res_map_script_t *into, yc_res_pro_script_type_t type
) {
    ...
    switch (type) {
        case YC_RES_PRO_SCRIPT_TYPE_SPATIAL: {
            ...
            if (0 == io->fread(&elevation_bytes, sizeof(unsigned char), 2, file)) {
                yc_res_map_parse_script_cleanup(into);
                return YC_RES_MAP_STATUS_IO;
            }
            ...
            uint16_t position = 0;
            if (0 == io->fread(&position, sizeof(uint16_t), 1, file)) {
                yc_res_map_parse_script_cleanup(into);
                return YC_RES_MAP_STATUS_IO;
            }
            ...
            if (0 == io->fread(&into->data.spatial->distance, sizeof(uint32_t), 1, file)) {
                yc_res_map_parse_script_cleanup(into);
                return YC_RES_MAP_STATUS_IO;
            }
        ...
        case YC_RES_PRO_SCRIPT_TYPE_TIMED: {
            ...
            if (0 == io->fread(&into->data.timed->duration, sizeof(uint32_t), 1, file)) {
                yc_res_map_parse_script_cleanup(into);
                return YC_RES_MAP_STATUS_IO;
            }
    ...
    uint32_t _flags = 0;
    if (0 == io->fread(&_flags, sizeof(uint32_t), 1, file)) {
        yc_res_map_parse_script_cleanup(into);
        return YC_RES_MAP_STATUS_IO;
    }
    ...
    if (0 == io->fread(&script_idx, sizeof(uint32_t), 1, file)) {
        yc_res_map_parse_script_cleanup(into);
        return YC_RES_MAP_STATUS_IO;
    }
    ...
    if (0 == io->fread(&into->object_id, sizeof(uint32_t), 1, file)) {
        yc_res_map_parse_script_cleanup(into);
        return YC_RES_MAP_STATUS_IO;
    }
    ...
    if (0 == io->fread(&lv_offset, sizeof(int32_t), 1, file)) {
        yc_res_map_parse_script_cleanup(into);
        return YC_RES_MAP_STATUS_IO;
    }
    lv_offset = yc_res_byteorder_int32(lv_offset);

    if (0 == io->fread(&lv_count, sizeof(int32_t), 1, file)) {
        yc_res_map_parse_script_cleanup(into);
        return YC_RES_MAP_STATUS_IO;
    }
    ...
    if (0 != io->fseek(file, sizeof(uint32_t) * 8, SEEK_CUR)) {
        yc_res_map_parse_script_cleanup(into);
        return YC_RES_MAP_STATUS_IO;
    }
