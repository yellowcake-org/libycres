...
yc_res_pro_status_t yc_res_pro_object_critter_parse(
    ...
    if (0 == api->fread(&flags_ext, sizeof(uint32_t), 1, file)) {
        yc_res_pro_critter_parse_cleanup(critter);
        return YC_RES_PRO_STATUS_IO;
    }
    flags_ext = yc_res_byteorder_uint32(flags_ext);

    if (0 == api->fread(&critter->script_id, sizeof(uint32_t), 1, file)) {
        yc_res_pro_critter_parse_cleanup(critter);
        return YC_RES_PRO_STATUS_IO;
    }
    critter->script_id = yc_res_byteorder_uint32(critter->script_id);

    if (0 == api->fread(&critter->sprite_id, sizeof(uint32_t), 1, file)) {
        yc_res_pro_critter_parse_cleanup(critter);
        return YC_RES_PRO_STATUS_IO;
    }
    critter->sprite_id = yc_res_byteorder_uint32(critter->sprite_id);

    if (0 == api->fread(&critter->packet_idx, sizeof(uint32_t), 1, file)) {
        yc_res_pro_critter_parse_cleanup(critter);
        return YC_RES_PRO_STATUS_IO;
    }
    critter->packet_idx = yc_res_byteorder_uint32(critter->packet_idx);

    if (0 == api->fread(&critter->team_idx, sizeof(uint32_t), 1, file)) {
        yc_res_pro_critter_parse_cleanup(critter);
        return YC_RES_PRO_STATUS_IO;
    }
    ...
    if (0 == api->fread(&critter->stats, sizeof(uint32_t), YC_RES_PRO_STAT_COUNT_SAVED, file)) {
        yc_res_pro_critter_parse_cleanup(critter);
        return YC_RES_PRO_STATUS_IO;
    }

    if (0 == api->fread(&critter->modifiers, sizeof(int32_t), YC_RES_PRO_STAT_COUNT_SAVED, file)) {
        yc_res_pro_critter_parse_cleanup(critter);
        return YC_RES_PRO_STATUS_IO;
    }
    ...
    if (0 == api->fread(&critter->skills, sizeof(int32_t), YC_RES_PRO_SKILL_COUNT, file)) {
        yc_res_pro_critter_parse_cleanup(critter);
        return YC_RES_PRO_STATUS_IO;
    }
    ...
    if (0 == api->fread(&critter->body, sizeof(uint32_t), 1, file)) {
        yc_res_pro_critter_parse_cleanup(critter);
        return YC_RES_PRO_STATUS_IO;
    }
    critter->body = yc_res_byteorder_uint32(critter->body);

    if (0 == api->fread(&critter->kill_reward, sizeof(uint32_t), 1, file)) {
        yc_res_pro_critter_parse_cleanup(critter);
        return YC_RES_PRO_STATUS_IO;
    }
    critter->kill_reward = yc_res_byteorder_uint32(critter->kill_reward);

    if (0 == api->fread(&critter->kill_type, sizeof(uint32_t), 1, file)) {
        yc_res_pro_critter_parse_cleanup(critter);
        return YC_RES_PRO_STATUS_IO;
    }
    ...
    api->fread(&critter->damage, sizeof(uint32_t), 1, file);
