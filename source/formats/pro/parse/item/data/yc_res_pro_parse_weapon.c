...
yc_res_pro_status_t yc_res_pro_object_item_weapon_parse(
    ...
    if (0 == io->fread(&anim_code, sizeof(uint32_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
    ...
    if (0 == io->fread(&weapon->damage_min, sizeof(uint32_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    weapon->damage_min = yc_res_byteorder_uint32(weapon->damage_min);

    if (0 == io->fread(&weapon->damage_max, sizeof(uint32_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    weapon->damage_max = yc_res_byteorder_uint32(weapon->damage_max);

    if (0 == io->fread(&weapon->damage_type, sizeof(uint32_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    weapon->damage_type = yc_res_byteorder_uint32(weapon->damage_type);

    if (0 == io->fread(&weapon->fire_primary.max_range, sizeof(uint32_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    weapon->fire_primary.max_range = yc_res_byteorder_uint32(weapon->fire_primary.max_range);

    if (0 == io->fread(&weapon->fire_secondary.max_range, sizeof(uint32_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    weapon->fire_secondary.max_range = yc_res_byteorder_uint32(weapon->fire_secondary.max_range);

    if (0 == io->fread(&weapon->projectile_pid, sizeof(uint32_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    weapon->projectile_pid = yc_res_byteorder_uint32(weapon->projectile_pid);

    if (0 == io->fread(&weapon->requirement.value, sizeof(uint32_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    ...
    if (0 == io->fread(&weapon->fire_primary.cost, sizeof(uint32_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    weapon->fire_primary.cost = yc_res_byteorder_uint32(weapon->fire_primary.cost);

    if (0 == io->fread(&weapon->fire_secondary.cost, sizeof(uint32_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    ...
    if (0 == io->fread(&critical_failure_idx, sizeof(int32_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    ...
    if (0 == io->fread(&weapon->perk, sizeof(yc_res_pro_perk_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    weapon->perk = yc_res_byteorder_int32(weapon->perk);

    if (0 == io->fread(&weapon->burst, sizeof(uint32_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    weapon->burst = yc_res_byteorder_uint32(weapon->burst);

    if (0 == io->fread(&weapon->caliber, sizeof(yc_res_pro_caliber_t), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
    ...
    if (0 == io->fread(&weapon->sound_id, sizeof(unsigned  char ), 1, file)) {
        yc_res_pro_weapon_parse_cleanup(weapon);
        return YC_RES_PRO_STATUS_IO;
    }
